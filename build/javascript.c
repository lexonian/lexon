...
#define backend_version "javascript 0.3.93 U"
...
bool js_certification(char **production, Certification * Certification,
		      int indent);
bool js_certify(char **production, Certify * Certify, int indent);
bool js_declaration(char **production, Declaration * Declaration, int indent);
bool js_declare(char **production, Declare * Declare, int indent);
bool js_filing(char **production, Filing * Filing, int indent);
bool js_file(char **production, File * File, int indent);
bool js_registration(char **production, Registration * Registration,
		     int indent);
bool js_register(char **production, Register * Register, int indent);
bool js_grantment(char **production, Grantment * Grantment, int indent);
bool js_grant(char **production, Grant * Grant, int indent);
bool js_appointment(char **production, Appointment * Appointment, int indent);
bool js_appoint(char **production, Appoint * Appoint, int indent);
...
bool js_document(char **production, Document *Document, int indent) {
	...
	if (opt_persistence) {
		...
		padcat(1, indent + 3, production,
		       ...
		       "'\");");
		padcat(1, indent + 3, production,
		       "var data = fs.readFileSync('", opt_persistence,
		       "', ()=>{});");
		padcat(1, indent + 3, production,
		       "var live = eval('(' + data + ')');");
...
bool js_covenant(char **production, Covenant *Covenant, int indent) {
	...
	padcat(1, indent + 2, &restorers, "var live = this.", list, "[id];");
	padcat(1, indent + 2, &restorers, "var instance = new ", class, "();");
...
bool js_provisions(char **production, Provisions *Provisions, int indent) {
	...
	if (uses_termination) padcat(1,
				     indent + (main_constructor_body ? 1 : 2),
...
bool js_actions(char **production, Actions *Actions, int indent) {
	...
	js_actions(production, Actions->Actions, indent);
	js_action(production, Actions->Action, indent);
...
bool js_action(char **production, Action *Action, int indent) {
	...
	if (current_function) {
		...
		//// visual optimization: padcat(0, 0, production, ") return \"not permitted.\";");
		padcat(0, 0, production, ") {");
		indent++;
	}
	...
	if (current_function) {
		...
		padcat(1, indent, production, "return 'not permitted.';");	// [2] ////// delete the dot (must refresh tests)
		padcat(1, --indent, production, "}");
	}
...
bool js_declaration(char **production, Declaration *Declaration, int indent) {
	...
	if (opt_log || opt_feedback) {
		padcat(1, indent, production, (!class ? "this" : "main"),
		       ".log(", recital_of_terms ? caller : "caller",
		       ", \"✓ ", Declaration->Symbol->Name, " declared\");");
		if (current_function) current_function->uses_caller |= class
				|| !strcmp(caller, "caller");
	}
	replace(production, "%preassign%", "");
...
bool js_appointment(char **production, Appointment *Appointment, int indent) {
	...
	if (opt_log || opt_feedback) {
		if (opt_feedback)
			padcat(1, indent, production,
			       (!class ? "this" : "main"), ".log(",
			       recital_of_terms ? caller : "caller", ", \"✓ ",
			       Appointment->Symbol->Name, " appointed\");");
		if (current_function)
			current_function->uses_caller |= class
				|| !strcmp(caller, "caller");
	}
	return true;
}
...
bool js_notification(char **production, Notification *Notification, int indent) {
	if (!Notification) return false;
	if (opt_debug) printf("producing Notification\n");
	padcat(1, indent, production, "( notification ");

	js_notify(production, Notification->Notify, indent + 1);
	js_object(production, Notification->Object, indent + 1);
	js_preposition(production, Notification->Preposition, indent + 1);
	js_expression(production, Notification->Expression, indent + 1);
